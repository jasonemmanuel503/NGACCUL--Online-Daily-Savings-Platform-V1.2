...
export class MTNDirectProvider implements PaymentProvider {
  ...
  async collect(
    ...
    // TODO: Implement direct MTN MoMo Collections API call here.
    // This will involve:
    // 1. Requesting an OAuth2 access token using MTN API partner/subscription credentials.
    // 2. Making a POST request to /collection/v1_0/requesttopay with the Amount, Party ID (phoneNumber), externalReference, etc.
    // 3. Polling or listening to callbacks for payment confirmation.
    throw new Error("MTN Direct payment provider is not yet enabled.");
  }

  async getStatus(reference: string): Promise<any> {
    // TODO: Implement direct MTN MoMo status check call here.
    // This will involve:
    // 1. Requesting an OAuth2 token if expired.
    // 2. Making a GET request to /collection/v1_0/requesttopay/{reference}.
    // 3. Checking the status in the response and returning a mapped standard format.
    throw new Error("MTN Direct payment provider is not yet enabled.");
  }
}
...
export class OrangeDirectProvider implements PaymentProvider {
  ...
  async collect(
    ...
    // TODO: Implement Orange Money Web Payment API call here.
    // This will involve:
    // 1. Getting access token from Orange Developer endpoint (POST /oauth/v3/token).
    // 2. Initializing Web Payment transaction (POST /orange_money/webpay/v1/webpayment).
    // 3. Mapping response containing pay_token and redirection url.
    throw new Error("Orange Direct payment provider is not yet enabled.");
  }

  async getStatus(reference: string): Promise<any> {
    // TODO: Implement Orange Money status check call here.
    // This will involve:
    // 1. Authenticating with Orange APIs.
    // 2. Making GET request to Orange Money transaction status endpoint with reference or transaction ID.
...
export class CampayProvider implements PaymentProvider {
  ...
  private async getCampayToken(): Promise<string> {
  ...
  async collect(
    ...
    const token = await this.getCampayToken();
    ...
    console.log(`[NGACCUL][Campay API Request] URL: ${this.apiBaseUrl}/collect/ Payload:`, payload);

    const collectRes = await fetch(`${this.apiBaseUrl}/collect/`, {
      method: "POST",
      headers: {
    ...
    if (!collectRes.ok) {
      ...
      throw new Error(`Campay service returned error (${collectRes.status}): ${errorText}`);
    }
  ...
  async getStatus(reference: string): Promise<{
    ...
    const statusRes = await fetch(`${this.apiBaseUrl}/transaction/${reference}/`, {
      method: "GET",
      headers: {
        "Authorization": `Token ${token}`,
        "Content-Type": "application/json"
    ...
    if (!statusRes.ok) {
      ...
      throw new Error(`Failed to check transaction status with Campay: ${errText}`);
    }
...
export class FuturaPayProvider implements PaymentProvider {
  ...
  // Falls back to generic placeholders — FuturaPay's own KYC on the phone number
  // is what actually matters for the mobile money push, these are just record-keeping.
  ...
  private async doInitiateAndProcess(
    ...
    // it's what gets credited to the member's savings balance downstream,
    // completely unaffected by this markup.
    //
    // Wrapped defensively: a fee-lookup failure should never block an actual
    // deposit, so we fall back to no markup rather than throwing.
    ...
    // --- Step 2 (per FuturaPay's official REST API docs: "Payment Status Update") ---
    // This step was previously missing, which is why /payments/processed was
    // rejecting requests with "Gateway ID is missing." FuturaPay needs to be told
    // which configured gateway (MTN Money vs Orange Money, etc.) this specific
    // transaction should route through before it can be processed.
    ...
    if (!statusUpdateRes.ok) {
      ...
      throw new Error(`FuturaPay status update failed (${statusUpdateRes.status}): ${errText}`);
    }
...
async function start() {
    ...
    try {
      ...
      if (table === "transactions" || table === "loans") {
        for (const rec of records) {
          const { data: existingRec } = await serverSupabase
            .from(table)
            .select("id")
            .eq("id", rec.id)
            .maybeSingle();

          // Only gate brand-new inserts by the depositing agent — never gate updates
          // to a transaction/loan that already exists (admin confirmations, status
          // changes, archiving, corrections, etc. must never be blocked by the
          // ORIGINAL creator's business hours).
          if (!existingRec) {
       ...
       if (table === "profiles") {
        for (const rec of records) {
          if (rec.phone) {
            const { data: duplicatePhone } = await serverSupabase
              .from("profiles")
              .select("id, full_name")
              .eq("phone", rec.phone)
              .neq("id", rec.id)
            ...
            if (duplicatePhone) {
              return res.status(400).json({
                success: false,
                error: `A user with this phone number is already registered (${duplicatePhone.full_name || "another member"}).`
              });
            }
          }
          ...
          // Server-authoritative unique identifier & code generation for new profile inserts
          const { data: existingProf } = await serverSupabase
            .from("profiles")
            .select("id, unique_display_id, agent_code, account_number")
            .eq("id", rec.id)
            .maybeSingle();

          if (!existingProf) {
            // Check Subdivision Access Padlock for non-Ngaoundéré subdivisions
            ...
            if (rec.role === "agent") {
              ...
              width = 3;
              defaultBase = 3;
            } else if (rec.role === "client") {
              prefix = "NGC-CLIENT-";
              width = 4;
              defaultBase = 43;
            } else if (rec.role === "branch_admin") {
              prefix = "NGC-ADMIN-";
              width = 3;
              defaultBase = 2;
            } else if (rec.role === "staff") {
              prefix = "NGC-STAFF-";
              width = 3;
              defaultBase = 1;
            } else if (rec.role) {
              prefix = `NGC-${rec.role.toUpperCase()}-`;
              width = 3;
              defaultBase = 1;
            }

            const { count, data: prefixData } = await serverSupabase
              .from("profiles")
              .select("unique_display_id", { count: "exact" })
              .filter("unique_display_id", "ilike", `${prefix}%`);

            let maxNum = 0;
            if (prefixData && prefixData.length > 0) {
              for (const item of prefixData) {
                if (item.unique_display_id) {
                  const parts = item.unique_display_id.split("-");
            ...
            const nextNum = Math.max((count || 0) + defaultBase, maxNum + 1);
            rec.unique_display_id = `${prefix}${nextNum.toString().padStart(width, "0")}`;

            if (rec.role === "agent") {
              let agentCode = "";
              let isAgentCodeUnique = false;
              let codeAttempts = 0;
              while (!isAgentCodeUnique && codeAttempts < 50) {
                agentCode = Math.floor(1000 + Math.random() * 9000).toString();
                const { data: existingCode } = await serverSupabase
                  .from("profiles")
                  .select("id")
                  .eq("agent_code", agentCode)
                  .maybeSingle();
                if (!existingCode) {
                  isAgentCodeUnique = true;
                }
                codeAttempts++;
              }
              rec.agent_code = agentCode;
            }
            ...
            if (rec.role === "client") {
              ...
              let accAttempts = 0;
              while (!isAccUnique && accAttempts < 50) {
                accNum = Math.floor(1000 + Math.random() * 9000).toString();
                const { data: existingAcc } = await serverSupabase
                  .from("profiles")
                  .select("id")
      ...
      while (attempt < MAX_RETRIES) {
        ...
        const errStr = (error.message || "") + " " + (error.code || "") + " " + (error.details || "");
        const isUniqueConflict = error.code === "23505" || errStr.includes("duplicate key") || errStr.includes("unique constraint") || errStr.includes("23505");

        if (isUniqueConflict && table === "profiles" && attempt < MAX_RETRIES - 1) {
          console.warn(`[Db Server Upsert] Unique constraint collision on profiles (attempt ${attempt + 1}). Regenerating IDs...`);
          for (const rec of records) {
            let prefix = "NGC-";
            let width = 3;
            let defaultBase = 1;
            ...
            const nextNum = Math.max((count || 0) + defaultBase, maxNum + 1) + attempt + 1;
            rec.unique_display_id = `${prefix}${nextNum.toString().padStart(width, "0")}`;

            if (rec.role === "agent") {
              rec.agent_code = Math.floor(1000 + Math.random() * 9000).toString();
            } else if (rec.role === "client") {
              rec.account_number = Math.floor(1000 + Math.random() * 9000).toString();
            }
          }
          attempt++;
          continue;
      ...
      if (upsertError) {
        console.error(`[Db Server Upsert Error] Table: ${table}, Message:`, upsertError.message);
        ...
        if (
          ...
          return res.status(400).json({
            success: false,
            error: `Database Schema Out of Sync: A required database column is missing from the database schema cache (${errMsg}). Please run pending migrations against your Supabase database and reload the PostgREST schema cache.`
          });
        }
  ...
  // Proxy endpoint to delete data
  app.post("/api/db/delete", requireInternalAuth, async (req, res) => {
    try {
      const { table, eqFilters } = req.body;
      if (!serverSupabase) {
        return res.status(503).json({ error: "Supabase not configured server-side" });
      }
      if (!table || !eqFilters) {
      ...
      let query = serverSupabase.from(table).delete();
      Object.entries(eqFilters).forEach(([key, val]) => {
        query = query.eq(key, val);
      });
      const { data, error } = await query;
      if (error) {
        console.error(`[Db Server Delete Error] Table: ${table}, Message:`, error.message);
        return res.status(200).json({ success: false, error: error.message });
      }
      return res.json({ success: true, data });
    } catch (err: any) {
      console.error("[Db Server Delete Exception]:", err);
      return res.status(500).json({ success: false, error: err.message });
    }
  });
  ...
  function getProviderForMethod(paymentMethod?: string): PaymentProvider {
    ...
    if (cleanMethod === "mtn") {
      if (paymentProviderMtn === "mtn_direct") return mtnDirectProvider;
      if (paymentProviderMtn === "campay") return campayProvider;
      return futurapayProvider;
    } else if (cleanMethod === "orange") {
      if (paymentProviderOrange === "orange_direct") return orangeDirectProvider;
      if (paymentProviderOrange === "campay") return campayProvider;
      return futurapayProvider;
    }
    // Default fallback
    return futurapayProvider;
  ...
  // 1. GET Campay config status (Backward Compatibility)
  ...
  // 2. POST /api/campay/collect (Backward Compatibility - Delegates to CampayProvider)
  app.post("/api/campay/collect", async (req, res) => {
    try {
      ...
      const result = await campayProvider.collect(amount, phoneNumber, description, externalReference);
  ...
  // 3. GET /api/campay/status/:reference (Backward Compatibility - Delegates to CampayProvider)
  app.get("/api/campay/status/:reference", async (req, res) => {
    try {
      ...
      const result = await campayProvider.getStatus(reference);
      return res.json(result);
  ...
  app.post("/api/payments/collect", async (req, res) => {
    try {
      ...
      // campay/mtn/orange all currently route through FuturaPay by default;
      // mtn/orange can be redirected via PAYMENT_PROVIDER_MTN/PAYMENT_PROVIDER_ORANGE env vars,
      // campay always uses the FuturaPay default.
...
Ensure absolutely no markdown wrapper like \`\`\`json outside the response - just return raw stringified JSON that can be parsed directly.`;
  ...
  async function sweepPendingReconciliations(): Promise<{ checked: number; results: any[] }> {
    if (!serverSupabase) {
      throw new Error("Supabase not configured server-side");
    }

    const { data: pending, error } = await serverSupabase
      .from("transactions")
      .select("id, status, payment_ref, payment_method, type, amount, client_id, branch_id, created_at")
      .in("status", ["pending", "rejected"])
      .eq("type", "deposit")
    ...
    for (const tx of pending || []) {
      try {
        ...
        // If it's pending/failed on the provider, don't reconcile if it's already rejected (user-cancelled)
        if ((normalized === "PENDING" || normalized === "FAILED") && tx.status === "rejected") {
          continue;
        }
  ...
  app.get("/api/cron/reconcile-payments", requireCronAuth, async (req, res) => {
    ...
    try {
      const outcome = await sweepPendingReconciliations();
  ...
  // Helper for E164 Phone Normalization (Cameroon)
  ...
  // Helper for SMS Gateway Integration with provider-priority chain:
  // 1. PRIMARY: Nexah (Username/Password) if SMS_GATEWAY_USERNAME & SMS_GATEWAY_PASSWORD are present.
  // 2. SECONDARY: Africa's Talking if SMS_GATEWAY_API_KEY & SMS_GATEWAY_USERNAME are present.
  // 3. FALLBACK: Simulation Mode if none are configured.
  async function sendSmsGateway(phone: string, message: string): Promise<{ success: boolean; simulated: boolean; raw?: any; provider?: string }> {
    let rawNexahToken = (process.env.NEXAH_API_TOKEN || "").trim();
    
    // Strip surrounding single/double quotes if present
    if (rawNexahToken.startsWith('"') && rawNexahToken.endsWith('"')) {
      rawNexahToken = rawNexahToken.slice(1, -1).trim();
    }
    if (rawNexahToken.startsWith("'") && rawNexahToken.endsWith("'")) {
      rawNexahToken = rawNexahToken.slice(1, -1).trim();
    }
    // Extract actual token if the user pasted it with a "Bearer " prefix
    if (rawNexahToken.toLowerCase().startsWith("bearer ")) {
      rawNexahToken = rawNexahToken.slice(7).trim();
    }

    const nexahToken = rawNexahToken;
    const nexahSenderId = (process.env.NEXAH_SENDER_ID || process.env.SMS_SENDER_ID || "NGACCUL").trim();

    const smsUsername = (process.env.SMS_GATEWAY_USERNAME || "").trim();
    const smsPassword = (process.env.SMS_GATEWAY_PASSWORD || "").trim();
    const smsApiKey = (process.env.SMS_GATEWAY_API_KEY || "").trim();
    const senderId = (process.env.SMS_SENDER_ID || "NGACCUL").trim();

    console.log(`[SMS Gateway Dispatch] Attempting send to ${phone}. Nexah configured: ${!!(smsUsername && smsPassword)}, Africa's Talking configured: ${!!(smsApiKey && smsUsername)}`);

    const errors: string[] = [];
    const formattedPhone = phone.replace(/\s+/g, "").replace(/\+/g, "");

    // 1. PRIMARY PATH: Nexah Username/Password API
    if (smsUsername && smsPassword) {
      const provider = "Nexah API";
      try {
        console.log(`[SMS Gateway] Routing via ${provider}. SenderID: "${senderId}", Mobile: "${formattedPhone}"`);

        const response = await fetch("https://smsvas.com/bulk/public/index.php/api/v1/sendsms", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
        ...
        console.log(`[SMS Gateway Response] HTTP Status: ${response.status}, Raw Body: ${textRes}`);
        ...
        const isSuccess = data?.responsecode === undefined || String(data.responsecode) === "200" || data?.responsecode === 0;

        if (isSuccess && response.status >= 200 && response.status < 300) {
          return { success: true, simulated: false, raw: data, provider };
        } else {
          throw new Error(`Gateway returned failure response: ${textRes}`);
        }
      } catch (err: any) {
        console.warn(`[SMS Gateway WARNING] ${provider} failed. Error: ${err.message || err}. Falling back...`);
        errors.push(`${provider} failed: ${err.message || err}`);
      }
    }

    // 2. SECONDARY PATH: Africa's Talking API
    if (smsApiKey && smsUsername) {
      const provider = "Africa's Talking API";
      try {
        const e164Phone = toE164Cameroon(phone);
        console.log(`[SMS Gateway] Routing via ${provider}. SenderID: "${senderId}", Mobile: "${e164Phone}"`);
        ...
        const isSuccess = data?.SMSMessageData?.Recipients?.some(
          (r: any) => r.status === "Success" || String(r.statusCode) === "101"
        ) || false;

        if (isSuccess && response.status >= 200 && response.status < 300) {
          return { success: true, simulated: false, raw: data, provider };
        } else {
          throw new Error(`Africa's Talking returned failure response: ${textRes}`);
        }
      } catch (err: any) {
        console.warn(`[SMS Gateway WARNING] ${provider} failed. Error: ${err.message || err}. Falling back...`);
        errors.push(`${provider} failed: ${err.message || err}`);
    ...
    // 3. UNCONFIGURED/FALLBACK PATH: Simulation Mode
    console.log(`\n======================================================`);
    console.log(`[SMS Simulation Mode] To: ${phone}`);
    console.log(`Message: ${message}`);
    if (errors.length > 0) {
      console.log(`Previous gateway failures:\n - ${errors.join("\n - ")}`);
    }
    console.log(`======================================================\n`);
    return { 
      success: true, 
      simulated: true, 
      provider: "Simulation",
      raw: errors.length > 0 ? { gateway_failures: errors } : undefined 
    };
  }
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    console.log("[NGACCUL][App System] Mounting dynamic Vite dev server assets system middleware.");
  } else {
    ...
    console.log("[NGACCUL][App System] Mounting bundled static built assets folder server.");
  }

  if (!process.env.VERCEL) {
    let RECONCILE_INTERVAL_MS = Number(process.env.RECONCILE_INTERVAL_MS) || 120000; // default 2 min, must be 60000–300000 range if set
    if (process.env.RECONCILE_INTERVAL_MS) {
      const val = Number(process.env.RECONCILE_INTERVAL_MS);
      if (val >= 60000 && val <= 300000) {
        RECONCILE_INTERVAL_MS = val;
      }
    }
    setInterval(() => {
      if (!serverSupabase) return;
      sweepPendingReconciliations().catch((e) => {
        console.error("[NGACCUL][Interval Reconcile] sweep failed:", e);
      });
    }, RECONCILE_INTERVAL_MS);
    console.log(`[NGACCUL][Interval Reconcile] VPS reconciliation loop active every ${RECONCILE_INTERVAL_MS}ms.`);
  }

  app.listen(PORT, "0.0.0.0", () => {
    console.log(`[NGACCUL][App System Server] Complete. Running actively on http://0.0.0.0:${PORT}`);
  });
}

start().catch((e) => {
  console.error("[NGACCUL][Fatal] uncaught startup server error:", e);
});
