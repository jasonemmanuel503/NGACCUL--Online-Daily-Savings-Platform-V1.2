...
-- 24. STAFF ROLES & PERMISSIONS EXPANSION
-- (merged from supabase_migration_staff_roles.sql) Idempotent: safe to run more than once.

-- 24.1 New permission keys
...
-- 24.3 Accountant's extra permission is NOT stored in a table row anywhere — verified it is only
...
--     profiles.permissions (TEXT[]) at staff-creation time. No SQL needed here.

-- 24.4 Loan Officer recommendation fields on loans
...
-- 24.5 Per-staff permission override (revoke path). profiles.permissions (TEXT[]) already exists
--     as the additive/grant path (referenced throughout db.ts and AdminApp.tsx as Profile.permissions).
--     This adds the mirror-image revoke path.
